...
1) Pipeline complet :
   - lecture Linkam
   - extraction q,I
   - renommage par température
   - waterfall global
   - split phases
   - waterfalls par phase
   - copie PNG dans _PLOTS

2) Mode manuel :
   - sélection de fichiers .dat
   - tracé direct d'un waterfall
"""
...
# =================================================
# 2) Lire Linkam : X (s) et Y (°C) — ROBUSTE
# =================================================
...
def waterfall_plot_like_user_script(
    ...
    motif_T = re.compile(r"_([-\d]+\.\d+)\.dat$", re.IGNORECASE)
    fichiers = sorted(
...
# =================================================
# 4b) Waterfall à partir d'une liste de fichiers
# =================================================
...
# =================================================
# 5) Extraction 2 colonnes + renommage
# =================================================
...
def extraire_2colonnes(in_path, out_path, log_fn):
    ...
    df.columns = ["q", "I"]
    df.to_csv(out_path, sep="\t", index=False, na_rep="nan")
    return True
...
def extraire_et_renommer(
    ...
    dest_extract = os.path.join(dossier_exp, f"{sample}_{kind}_2colonnes")
    ...
    for f in sorted(os.listdir(dossier_exp)):
        if not f.lower().endswith(".dat"):
            continue
        if ignore_non_sample and (not f.startswith(sample)):
            continue
    ...
    if not sources:
        log_fn(f"⚠️ Aucun fichier {kind} dans {dossier_exp}")
        return None
...
def ranger_par_phases(
    ...
    if not segs:
        log_fn("⚠️ Détection phases impossible.")
        return []
...
# =================================================
# 7) Waterfall par phase + copie dans _PLOTS
# =================================================
...
# =================================================
# 9) Traitement dossier expérience
# =================================================
def traiter_un_dossier_experience(
    ...
    log(
        f"DEBUG: dX={dX:.6f}s  step_per_image={step_per_image}  "
    ...
    if dossier_saxs:
        png = os.path.join(dossier_exp, f"{sample}_SAXS.png")
        waterfall_plot_like_user_script(
            dossier_saxs,
            f"{sample} - SAXS",
            png,
    ...
    if dossier_waxs:
        png = os.path.join(dossier_exp, f"{sample}_WAXS.png")
        waterfall_plot_like_user_script(
            dossier_waxs,
            f"{sample} - WAXS",
            png,
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Pipeline SWING SAXS/WAXS + Fichier Température")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            header,
            text="PipPipeline SWING SAXS/WAXS + Fichier Température",
        ...
        ttk.Radiobutton(mode_inner, text="Pipeline complet", variable=self.var_mode, value="pipeline").pack(side="left", padx=8)
        ttk.Radiobutton(mode_inner, text="Tracer des fichiers sélectionnés", variable=self.var_mode, value="manual").pack(side="left", padx=8)
        ...
        fields = [
            (
                "step_per_image",
                self.var_step,
                "Nombre de ligne Linkam correspondant à une image SAXS/WAXS.\n"
                "Exemple : si dX ≈ 2 s et 1 image = 10 s, alors step_per_image = 5."
            ),
            (
                "pas",
                self.var_pas,
                "Trace une courbe sur 'pas' fichiers.\n"
            ...
            (
                "dpi",
                self.var_dpi,
                "Résolution des images PNG exportées.\n"
                
            ),
            (
                "eps_plateau",
                self.var_eps,
                "Seuil de variation de température pour détecter un plateau.\n"
              
            ),
        ]

        for i, (label, var, info) in enumerate(fields):
            row = i // 3
            col = (i % 3) * 2
            self._add_label_with_info(params, f"{label} :", info, row, col, pad)
            ttk.Entry(params, textvariable=var, width=12).grid(row=row, column=col + 1, sticky="w", **pad)

        manual_frame = ttk.Frame(params)
        manual_frame.grid(row=2, column=0, columnspan=6, sticky="w", padx=6, pady=(8, 0))
        ...
        opts.grid(row=3, column=0, columnspan=6, sticky="w", padx=6, pady=(8, 0))
        ...
        cb2 = ttk.Checkbutton(opts, text="Split phases", variable=self.var_split)
        cb2.pack(side="left", padx=8)
        info2 = ttk.Label(opts, text=" ⓘ", style="Info.TLabel", cursor="hand2")
        info2.pack(side="left")
        ToolTip(info2, "Sépare automatiquement les fichiers en heat1, cool1, heat2, cool2 et plateau.")
    ...
    def run_clicked(self):
        ...
        def worker():
            try:
                ...
                self.log(f"pas = {p['pas']}")
                self.log(f"decal_x = {p['decal_x']}")
                self.log(f"decal_log10 = {p['decal_log10']}")
                self.log(f"dpi = {p['dpi_save']}")
                self.log("")

                if p["mode"] == "pipeline":
                    self.log(f"CHEMIN = {p['chemin']}")
                    self.log(f"step_per_image = {p['step_per_image']}")
                    self.log(f"eps_plateau = {p['eps_plateau']}")
                    self.log("")
